...
fun EditPersonalDetailsScreen() {
    ...
    val viewModel = koinViewModel<EditPersonalDetailsViewModel>()

    val state by viewModel.collectAsStateAndEffects { effect ->
        when (effect) {
            EditPersonalDetailsViewModel.Effect.NavigateBack -> appNavigator.navigateBack()
        }
    }
...
@Composable
private fun EditPersonalDetailsContent(
